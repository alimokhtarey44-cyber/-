...
const UIManager = {
  ...
  buildStaticInstruments() {
    ...
    const pg = $('powerGrid');
    pg.appendChild(makeInstrumentDOM('voltmeter', 'VOLTAGE'));
    pg.appendChild(makeInstrumentDOM('ammeter', 'CURRENT'));
    pg.appendChild(makeInstrumentDOM('powermeter', 'POWER'));
    // network bars
  ...
  renderActivePage() {
    ...
    if (page === 'battery') {
      ...
      const pwrFresh = ExternalPowerManager.connected && (nowTs() - ExternalPowerManager.lastUpdate < 8000);
      const v = pwrFresh ? ExternalPowerManager.voltage : null;
      const i = pwrFresh ? ExternalPowerManager.current : null;
      updateInstrumentDOM('voltmeter', { value: v, status: v !== null ? 'available' : 'unavailable', source: 'external_detector', decimals: 2, unit: 'V', gaugeOpts: { min: 0, max: 30 } });
      updateInstrumentDOM('ammeter', { value: i, status: i !== null ? 'available' : 'unavailable', source: 'external_detector', decimals: 2, unit: 'A', gaugeOpts: { min: 0, max: 5 } });
      const power = (v !== null && i !== null) ? v * i : null;
      updateInstrumentDOM('powermeter', { value: power, status: power !== null ? 'available' : 'unavailable', source: 'external_detector', decimals: 2, unit: 'W', gaugeOpts: { min: 0, max: 50 } });
      if (power !== null) $('inst-powermeter').querySelector('[data-role="source"]').textContent = 'SOURCE: CALCULATED (V×I)';
    }
    if (page === 'network') {
      const conn = navigator.connection || navigator.mozConnection || navigator.webkitConnection;
...
/* ============================================================ FoldDemo ============================================================ */
const FoldDemo = {
  mode: 'sensor', baseline: 0, manualAngle: 0,
  init() {
    $('foldModeBtn').addEventListener('click', () => {
      this.mode = this.mode === 'sensor' ? 'manual' : 'sensor';
      $('foldModeBtn').textContent = this.mode === 'sensor' ? 'SWITCH TO MANUAL' : 'SWITCH TO SENSOR';
      $('foldModeReadout').textContent = this.mode.toUpperCase();
      $('foldSliderWrap').classList.toggle('hidden', this.mode !== 'manual');
    });
    $('foldRecalBtn').addEventListener('click', () => {
      const raw = (SX.rawNum.orientation && SX.rawNum.orientation.roll) || 0;
      this.baseline = raw; this.manualAngle = 0; $('foldSlider').value = 0;
    });
    $('foldSlider').addEventListener('input', (e) => { this.manualAngle = parseFloat(e.target.value); });
  },
  render() {
    let angle;
    if (this.mode === 'sensor') {
      const raw = (SX.rawNum.orientation && SX.rawNum.orientation.roll) || 0;
      angle = clamp(raw - this.baseline, -45, 45);
    } else angle = this.manualAngle;
    $('foldRight').style.transform = `rotateY(${-angle}deg)`;
    $('foldShade').style.opacity = Math.min(Math.abs(angle) / 45, 1) * 0.9;
    $('foldAngleReadout').textContent = fmt(angle, 0) + '°';
  }
};
